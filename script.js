...
const swiperAnimationConfig = {
  ...
  function tCount(nounKey, count) {
    ...
    let cat;
    try { cat = pr.select(count); } catch { cat = count === 1 ? 'one' : 'other'; }
  ...
  function ensureFlatpickrLocales() {
    ...
    if (!l.fr) {
      l.fr = { firstDayOfWeek: 1, weekdays: { shorthand: ["Dim","Lun","Mar","Mer","Jeu","Ven","Sam"], longhand: ["Dimanche","Lundi","Mardi","Mercredi","Jeudi","Vendredi","Samedi"] }, months: { shorthand: ["Janv","Févr","Mars","Avr","Mai","Juin","Juil","Août","Sept","Oct","Nov","Déc"], longhand: ["Janvier","Février","Mars","Avril","Mai","Juin","Juillet","Août","Septembre","Octobre","Novembre","Décembre"] }, rangeSeparator: ' au ', weekAbbreviation: 'Sem', scrollTitle: 'Défiler pour augmenter', toggleTitle: 'Cliquer pour basculer' };
    }
    if (!l.it) {
      l.it = { firstDayOfWeek: 1, weekdays: { shorthand: ["Dom","Lun","Mar","Mer","Gio","Ven","Sab"], longhand: ["Domenica","Lunedì","Martedì","Mercoledì","Giovedì","Venerdì","Sabato"] }, months: { shorthand: ["Gen","Feb","Mar","Apr","Mag","Giu","Lug","Ago","Set","Ott","Nov","Dic"], longhand: ["Gennaio","Febbraio","Marzo","Aprile","Maggio","Giugno","Luglio","Agosto","Settembre","Ottobre","Novembre","Dicembre"] }, rangeSeparator: ' al ', weekAbbreviation: 'Sett', scrollTitle: 'Scorri per aumentare', toggleTitle: 'Clicca per alternare' };
    }
  }
  ...
  function updateChildAgeItems(childCount) {
    ...
    items.forEach((item, idx) => {
      ...
      if (show) {
        ...
        inputs.forEach((inp) => {
          if (inp.dataset.originalName && !inp.name) inp.name = inp.dataset.originalName;
          inp.disabled = false;
        });
        ...
        inputs.forEach((inp) => {
          if (!inp.dataset.originalName) inp.dataset.originalName = inp.getAttribute('name') || '';
          inp.value = '';
          inp.disabled = true;
  ...
  // Max-Limit und Button-State-Helfer (wie in script.js)
        ...
        setTimeout(function() {
          ...
          if (heroRequestBtn) {
            heroRequestBtn.addEventListener('click', function() {
              ...
              // Altersfelder gemäß Kinderanzahl aktualisieren
      ...
      (function initFormPersons() {
        ...
        cCtrls.forEach(function(ctrl) {
          ctrl.addEventListener('click', function() {
            ...
            // clamp nach vorhandenen Items
            (function(){
              const max = (document.querySelectorAll('[data-child-age-item]').length) || Infinity;
              if (formChildrenCount < 0) formChildrenCount = 0;
              if (max !== Infinity && formChildrenCount > max) formChildrenCount = max;
            })();
            const valC = tCount('children', formChildrenCount);
            if (formChildsText) {
              if (formChildsText.value !== undefined) formChildsText.value = valC;
            ...
            // Altersfelder aktualisieren
            updateChildAgeItems(formChildrenCount);
            // Plus-State
            const plusB = ctrl.closest('.form_picker-persons-wrapper').querySelector('[data-controls="plus"]');
            const max = (document.querySelectorAll('[data-child-age-item]').length) || Infinity;
            if (plusB) {
              if (max !== Infinity && formChildrenCount >= max) plusB.classList.add('is-disabled');
              else plusB.classList.remove('is-disabled');
            }
          });
        });
        // Initialzustand Altersfelder
        updateChildAgeItems(formChildrenCount);
      })();
